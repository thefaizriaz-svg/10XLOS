  ...
  return `
    <div class="card goal-card ${isDone ? 'completed-card' : ''}" data-goal="${g.id}">
      ...
      ${auto ? `
        ...
        <div class="progress-hint">${auto.done} of ${auto.total} actions completed \u2014 measured automatically. Untick one below to reopen this goal.</div>
      ` : `
    ...
    document.querySelectorAll('[data-reopen-goal]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (linkedActions.length) {
          const lastCompleted = linkedActions.filter(a => a.status === 'Completed').slice(-1)[0];
          if (lastCompleted) lastCompleted.status = 'Active';
        } else {
