import { STORE, migrate, getStatus, isSecured, getUserEmail, secureAccount, signInToRestore } from './data.js';
    ...
    <div class="card goal-card" data-goal="${g.id}">
      <div class="goal-top">
        <div>
          ...
          ${dl ? `<div class="deadline-line ${dl.overdue ? 'overdue' : ''} ${dl.done ? 'done' : ''}">${dl.dateLabel} · ${dl.text}</div>` : ''}
        </div>
      ...
      ${auto ? `
        ...
        <div class="progress-hint">${auto.done} of ${auto.total} actions completed \u2014 measured automatically</div>
      ` : `
        ...
        <div class="progress-hint">Add actions below to measure this automatically</div>
      `}
      <div class="linked-actions">
        ...
        `).join('') : `<div class="empty-state small">No actions linked yet.</div>`}
      </div>
...
function renderCompletedGoalCard(g) {
  const dateLabel = g.completedAt ? new Date(g.completedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : '';
  return `
    <div class="card goal-card completed-card" data-goal="${g.id}">
      <div class="goal-top">
        <div>
          <div class="goal-title strike">${escapeHtml(g.title)}</div>
          <div class="goal-why">${escapeHtml(g.why)}</div>
          <div class="deadline-line done">Completed ${dateLabel}</div>
        </div>
        <div class="card-actions">
          <button class="icon-btn" data-edit-goal="${g.id}" aria-label="Edit goal">${ICONS.edit}</button>
          <button class="icon-btn danger" data-delete-goal="${g.id}" aria-label="Delete goal">${ICONS.trash}</button>
...
function renderGoals(state) {
  ...
  return `
    ...
    ${completed.length ? completed.map(g => renderCompletedGoalCard(g)).join('') : `<div class="empty-state small">Finished goals will land here automatically.</div>`}
...
function wireGenericEditables(container) {
  ...
  container.querySelectorAll('.add-list-item').forEach(btn => {
...
async function saveModal() {
  ...
  if (type === 'goal') {
    ...
    if (editing.id) {
      ...
      a.title = title; a.why = why; a.goalId = goalId; a.status = status;
      logChange(state, `Updated action "${title}"`);
    } else {
      state.actions.push({ id: 'a' + Date.now(), title, why, goalId, status });
